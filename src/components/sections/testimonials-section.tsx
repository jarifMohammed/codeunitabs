...
function TestimonialCard({ testimonial }: { testimonial: typeof testimonials[number] }) {
  return (
    <motion.article
      className="relative flex w-[474.4px] max-w-[calc(100vw-2rem)] flex-col gap-5 overflow-hidden rounded-xl border border-white/[0.06] bg-sectionCard/40 p-5 backdrop-blur-xl sm:gap-6 sm:p-[33px] shrink-0"
      whileHover={{
        y: -4,
...
function MarqueeRow({ items, reverse = false, duration = 28 }: { items: typeof testimonials; reverse?: boolean; duration?: number }) {
  ...
  return (
    <motion.div
      ...
      animate={isPaused ? undefined : { x: reverse ? ["-50%", "0%"] : ["0%", "-50%" ] }}
      initial={reverse ? { x: "-50%" } : { x: "0%" }}
      transition={{
        duration,
        repeat: Infinity,
        ease: "linear",
    ...
    >
      {doubledItems.map((testimonial, index) => (
...
export function TestimonialsSection() {
  return (
    <section className="overflow-hidden bg-bg py-20 md:py-24 xl:py-[120px]" id="insights">
      ...
      <AnimateIn type="fade-up" delay={120} className="relative flex flex-col gap-8">
        <div className="pointer-events-none absolute inset-y-0 left-0 z-10 w-24 bg-gradient-to-r from-bg to-transparent" />
        <div className="pointer-events-none absolute inset-y-0 right-0 z-10 w-24 bg-gradient-to-l from-bg to-transparent" />
        <MarqueeRow items={testimonials} />
        <MarqueeRow items={testimonials} reverse duration={32} />
      </AnimateIn>
    </section>
  );
}
